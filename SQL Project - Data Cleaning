...
-- now that I have this we can delete rows were row_num is greater than 2
...
-- When I look at industry it looks like there are some null and empty rows.
...
-- I'm sure it's the same for the others. What I did is
-- write a query that if there is another row with the same company name, it will update it to the non-null industry values
-- makes it easy so if there were thousands I wouldn't have to manually check them all

-- I want to set the blanks to nulls since those are typically easier to work with
UPDATE world_layoffs.layoffs_staging2
SET industry = NULL
WHERE industry = '';

-- now when I check those are all null
...
-- I also noticed the Crypto has multiple different variations. I need to standardize that - I set all to Crypto
SELECT DISTINCT industry
FROM world_layoffs.layoffs_staging2
ORDER BY industry;
...
-- everything looks good except apparently there are some "United States" and some "United States." with a period at the end. I will standardize this.
...
-- now when I run this again it is fixed
SELECT DISTINCT country
FROM world_layoffs.layoffs_staging2
ORDER BY country;
...
-- 4. remove any columns and rows I need to
